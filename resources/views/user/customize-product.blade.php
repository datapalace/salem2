                ...
                <div class="col-lg-7">
                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-4 col-md-4 col-sm-3 mb-mobile"><span
                                class="bytext color-gray-500 font-xs font-large">{{ $product->style_code }}</span>
                                                ...
                                                <div class="d-flex flex-row align-items-start gap-3">
                                                    ...
                                                    <div id="customCanvasToolbar" class="mb-2 d-flex flex-column" style="gap: 8px; min-width:180px;">
                                                        ...
                                                        <div class="btn-group mb-1" role="group" aria-label="Text Styles">
                                                            <button type="button" id="toggleBold" class="btn btn-outline-secondary btn-sm">B</button>
                                                            <button type="button" id="toggleItalic" class="btn btn-outline-secondary btn-sm"><i>I</i></button>
                                                            <button type="button" id="toggleUnderline" class="btn btn-outline-secondary btn-sm" style="text-decoration:underline;">U</button>
                                                            <button type="button" id="undoBtn" class="btn btn-outline-primary btn-sm" title="Undo"><i class="fa fa-undo"></i></button>
                                                            <button type="button" id="redoBtn" class="btn btn-outline-primary btn-sm" title="Redo"><i class="fa fa-redo"></i></button>
                                                        </div>
                                    ...
                                    if (activeObject && activeObject.type === 'i-text') {
                                        switch (this.value) {
                                            ...
                                            case 'underline':
                                                ...
                                                break;
                                            case 'red-text':
                                                activeObject.set('fill', activeObject.fill === '#ff0000' ? '#222' : '#ff0000');
                                                break;
                                            case 'large-text':
                                                activeObject.set('fontSize', activeObject.fontSize === 36 ? 24 : 36);
                                                break;
                                            case 'medium-text':
                                                activeObject.set('fontSize', activeObject.fontSize === 24 ? 16 : 24);
                                                break;
                                            case 'small-text':
                                                activeObject.set('fontSize', activeObject.fontSize === 12 ? 24 : 12);
                                                break;
                                            default:
                                                // Reset styles
                                                activeObject.set({
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function saveState() {
            if (window.customCanvas) {
                undoStack.push(JSON.stringify(customCanvas));
        ...
        if (window.customCanvas) {
            customCanvas.on('object:added', saveState);
            customCanvas.on('object:modified', saveState);
            customCanvas.on('object:removed', saveState);
        }
        ...
        document.getElementById('undoBtn').addEventListener('click', function() {
            if (undoStack.length > 1) {
                ...
                customCanvas.loadFromJSON(undoStack[undoStack.length - 1], function() {
        ...
        document.getElementById('redoBtn').addEventListener('click', function() {
            if (redoStack.length > 0) {
                ...
                customCanvas.loadFromJSON(state, function() {
                    customCanvas.renderAll();
                });
            }
        });
        ...
        document.getElementById('toggleBold').addEventListener('click', function() {
            const obj = customCanvas.getActiveObject();
